...
from facebook.ads import create_image_ad, create_video_ad
...
class AdCampaignAgent:
    ...
    def run(self):
        ...
        # Step 9: META UPLOADER — MULTI FILE LOOP (Images + Videos)
        print("\n📤 Uploading creatives to Meta...")
                ...
                try:
                    ...
                    # -----------------------------
                    if ext in image_ext:
                        res = upload_media_service(account_id, "image", access_token, file_path)
                        
                        if "error" in res:
                             print(f"❌ Upload failed: {res['error']}")
                             continue
                             
                        image_hash = res.get('image_hash')

                        # Create Ad
                        ad = asyncio.run(create_image_ad(
                            account_id=account_id,
                            adset_id=adset_id,
                            page_id=page_id,
                            ad_name=f"Auto Image Ad - {file_name}",
                        ...
                        print("✅ Image Ad Created:", ad)

                    # -----------------------------
                    # VIDEO UPLOAD
                    # -----------------------------
                    elif ext in video_ext:
                        res = upload_media_service(account_id, "video", access_token, file_path)
                        ...
                        # Create Ad
                        ad = asyncio.run(create_video_ad(
                            account_id=account_id,
                            adset_id=adset_id,
                            page_id=page_id,
                            ad_name=f"Auto Video Ad - {file_name}",
                            video_id=video_id,
                            thumbnail_hash="", # Optional
                            message="Watch now!",
                            link="https://example.com",
                            access_token=access_token
                        ))

                        print("🎬 Video Ad Created:", ad)
            ...
            print("⚠️ Skipped Meta upload. Missing folder path, adset_id, or env vars.")
